...
void loop()
{
  ...
  if (digitalRead(BTN_PIN) == 0) // pressed then snap
  {
    if ((millis() - prev_millis > 500) && (press_state == false))
    {
      // take snapshot
      uint32_t jpg_sz = hw_camera_jpg_snapshot(buffer); // photo size
      ESP_LOGI(TAG, "JPEG Size: %u", jpg_sz);
